...
from common.tools import district_search,around_search,driving_route
from common.prompts import (AROUND_SEARCH_ROUTER_SYSTEM_PROMPT,
PATH_PLANNING_ROUTER_SYSTEM_PROMPT,CLASSIFY_QUERY_PROMPT)
...
async def call_around_search_agent(state: Gaodemap_State_Router):
    ...
    return {"results": [{"source": "around_search_agent", "result": result["messages"][-1].content}]}
...
async def call_path_planning_agent(state: Gaodemap_State_Router):
    ...
    return {"results": [{"source": "path_planning_agent", "result": result["messages"][-1].content}]}
...
workflow = (
    ...
    .add_conditional_edges("classify", route_to_agents, ["around_search_agent", "path_planning_agent"])
