...
from common.tools import (district_search, district_deepthink, geocode, around_search, calculate_distance, walking_route, driving_route, guiding_deepthink)
from common.prompts import (DISTRICT_SEARCH_SUBAGENT_THINKING_SYSTEM_PROMPT, AROUND_SEARCH_SUBAGENT_THINKING_SYSTEM_PROMPT, PATH_PLANNING_SUBAGENT_THINKING_SYSTEM_PROMPT, TRAVEL_GUIDE_SUBAGENT_THINKING_SYSTEM_PROMPT, MAIN_SUBAGENT_THINKING_SYSTEM_PROMPT)
...
async def call_district_search_agent(query: str):
    ...
    """
    result = await district_search_agent.ainvoke({"messages": [{"role": "user", "content": query}]})
...
async def retry_geocode(
    ...
    if request.tool_call['name'] == 'geocode':
        try:
            ...
            return ToolMessage(content=f"工具调用失败，错误信息是：{e}。请你一定要保证传入的地点名称是全名，不要使用简称。",tool_call_id=request.tool_call['id'])
    return await handler(request)
...
async def call_travel_guide_agent(query: str):
    ...
    """
    result = await travel_guide_agent.ainvoke({"messages": [{"role": "user", "content": query}]})
    return result["messages"][-1].content
