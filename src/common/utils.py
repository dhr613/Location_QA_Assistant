...
async def _extract_detailed_poi(result: dict) -> dict:
    ...
    poi_data = {
        ...
        if detail.get("navi_location")
        else None,
        "brand": detail.get("brand"),
        "price": float(detail["price"])
        if detail.get("price") and detail["price"].replace(".", "", 1).isdigit()
        else None,
        "overall_rating": float(detail["overall_rating"])
        if detail.get("overall_rating")
        and detail["overall_rating"].replace(".", "", 1).isdigit()
        else None,
        "taste_rating": float(detail["taste_rating"])
        if detail.get("taste_rating")
        and detail["taste_rating"].replace(".", "", 1).isdigit()
        else None,
        "service_rating": float(detail["service_rating"])
        if detail.get("service_rating")
        and detail["service_rating"].replace(".", "", 1).isdigit()
        else None,
        "environment_rating": float(detail["environment_rating"])
        if detail.get("environment_rating")
        and detail["environment_rating"].replace(".", "", 1).isdigit()
        else None,
...
async def gaode_parse_key_words_and_around_search(data):
    ...
    for poi in pois:
        ...
        if isinstance(photos_obj, dict) and "url" in photos_obj:
            ...
            photo_url = (
                photos_list[0].get("url")
                if isinstance(photos_list[0], dict) and "url" in photos_list[0]
                else None
            )
        else:
            photo_url = None
        ...
        distance_m = (
            int(distance) if isinstance(distance, str) and distance.isdigit() else None
        )
        ...
        simplified = {
            ...
            if cost and str(cost).replace(".", "", 1).isdigit()
            else None,
            "rating": float(rating)
        ...
        # 过滤掉值为 None 或 [] 的项
        filtered = {k: v for k, v in simplified.items() if v is not None and v != []}
...
async def gaode_parse_polygon_search(data):
    ...
    for poi in pois:
        ...
        try:
            lng, lat = (
                map(float, location_str.split(",")) if location_str else (None, None)
            )
        except (ValueError, AttributeError):
            lng, lat = None, None
        ...
        item = {
            ...
            "cost_per_person": float(cost)
            if cost and cost.replace(".", "", 1).isdigit()
            else None,
            "rating": float(rating)
        ...
        # 过滤掉值为 None 或 [] 的项
        filtered = {k: v for k, v in item.items() if v is not None and v != []}
...
async def gaode_parse_poi_search(poi_dict: dict) -> dict:
    ...
    if pois_list and isinstance(pois_list[0], dict):
        ...
        filtered_poi_detail = {
            k: v for k, v in poi_detail.items() if v is not None and v != []
        }
...
async def gaode_parse_geocode(data):
    ...
    for position in position_list:
        ...
        location_list.append(
            {"name": formatted_address, "longitude": lng, "latitude": lat}
        )
